...
class AutoPayResourceIT {
    ...
    @Test
    @Transactional
    void getAllAutoPaysByCustBillArrangementIsEqualToSomething() throws Exception {
        // Initialize the database
        autoPayRepository.saveAndFlush(autoPay);
        CustBillArrangement custBillArrangement = CustBillArrangementResourceIT.createEntity(em);
        em.persist(custBillArrangement);
        em.flush();
        autoPay.setCustBillArrangement(custBillArrangement);
        custBillArrangement.setAutoPay(autoPay);
        autoPayRepository.saveAndFlush(autoPay);
        Long custBillArrangementId = custBillArrangement.getId();

        // Get all the autoPayList where custBillArrangement equals to custBillArrangementId
        defaultAutoPayShouldBeFound("custBillArrangementId.equals=" + custBillArrangementId);

        // Get all the autoPayList where custBillArrangement equals to (custBillArrangementId + 1)
        defaultAutoPayShouldNotBeFound("custBillArrangementId.equals=" + (custBillArrangementId + 1));
    }
    ...
    void putNewAutoPay() throws Exception {
        ...
        updatedAutoPay.channel(UPDATED_CHANNEL).autoPayId(UPDATED_AUTO_PAY_ID).debitDate(UPDATED_DEBIT_DATE).status(UPDATED_STATUS);
    ...
    void partialUpdateAutoPayWithPatch() throws Exception {
        ...
        partialUpdatedAutoPay.autoPayId(UPDATED_AUTO_PAY_ID).debitDate(UPDATED_DEBIT_DATE);
    ...
    void fullUpdateAutoPayWithPatch() throws Exception {
        ...
        partialUpdatedAutoPay.channel(UPDATED_CHANNEL).autoPayId(UPDATED_AUTO_PAY_ID).debitDate(UPDATED_DEBIT_DATE).status(UPDATED_STATUS);
