...
public class IndividualServiceImpl implements IndividualService {
    ...
    /**
     *  Get all the individuals where ShoppingSessionRef is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<IndividualDTO> findAllWhereShoppingSessionRefIsNull() {
        log.debug("Request to get all individuals where ShoppingSessionRef is null");
        return StreamSupport
            .stream(individualRepository.findAll().spliterator(), false)
            .filter(individual -> individual.getShoppingSessionRef() == null)
            .map(individualMapper::toDto)
