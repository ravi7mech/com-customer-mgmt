...
public class AutoPayServiceImpl implements AutoPayService {
    ...
    /**
     *  Get all the autoPays where CustBillArrangement is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<AutoPayDTO> findAllWhereCustBillArrangementIsNull() {
        log.debug("Request to get all autoPays where CustBillArrangement is null");
        return StreamSupport
            .stream(autoPayRepository.findAll().spliterator(), false)
            .filter(autoPay -> autoPay.getCustBillArrangement() == null)
            .map(autoPayMapper::toDto)
