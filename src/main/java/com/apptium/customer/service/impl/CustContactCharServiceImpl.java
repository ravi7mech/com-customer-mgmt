...
public class CustContactCharServiceImpl implements CustContactCharService {
    ...
    /**
     *  Get all the custContactChars where CustContact is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<CustContactCharDTO> findAllWhereCustContactIsNull() {
        log.debug("Request to get all custContactChars where CustContact is null");
        return StreamSupport
            .stream(custContactCharRepository.findAll().spliterator(), false)
            .filter(custContactChar -> custContactChar.getCustContact() == null)
            .map(custContactCharMapper::toDto)
            .collect(Collectors.toCollection(LinkedList::new));
    }
